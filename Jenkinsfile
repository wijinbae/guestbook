...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="wijinbae/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/wijinbae/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_wijinbae') {
                        oDockImage.push()
                    }
                }
            }
